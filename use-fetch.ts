...
type UseFetchFn<R extends Rpc, T> = (rpc: R, ...args: any[]) => Promise<T> | null;
...
type UseFetchClientOptions<T, Mapped> = {
	deps?: any[];
	depsDebounce?: number;
	ignoreAbort?: boolean;
...
const validateOptions = <T, Mapped>(options: UseFetchClientOptions<T, Mapped>): void => {
	if (!isObject(options)) {
...
const useFetch = <R extends Rpc, T, Mapped = T>(
	...
	try {
		...
		throw new Error('app.useFetch failed to start due to invalid options: ' + (err as Error).message);
	}
	...
	const fetch = useCallback(
		async (...args: any[]): Promise<Mapped | null> => {
			...
			try {
				...
				const promise = fnRef.current(rpc as R, ...args);
	...
	useEffect(() => {
		...
		// priorize triggerDeps if exists
...
const useLazyFetch = <R extends Rpc, T, Mapped = T>(
	fn: UseFetchFn<R, T>,
	options?: {
		ignoreAbort?: boolean;
